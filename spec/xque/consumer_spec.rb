  ...
  describe "#run_once" do
    ...
    it "performs a job and deletes it" do
      allow_any_instance_of(ConsumerTestWorker).to receive(:perform) do |worker|
        RedisClient.hset("result", attribute1: worker.attribute1, attribute2: worker.attribute2)
      end
      ...
      expect(RedisClient.hgetall("result")).to eq("attribute1" => "value1", "attribute2" => "value2")
      expect(RedisClient.zcard("xque:pending:items")).to eq(0)
      expect(RedisClient.hlen("xque:jobs")).to eq(0)
    end
    ...
    it "adds popped jobs to the pending list with correct expiry" do
      ...
      allow_any_instance_of(ConsumerTestWorker).to receive(:perform) do
        pending = RedisClient.zrange("xque:pending:items", 0, 10, withscores: true)
      end
    ...
    it "pops jobs from the pending list when expired" do
      ...
      RedisClient.zadd("xque:pending:items", RedisClient.time[0] - 5, job_id)
    ...
    it "does not pop jobs from the pending list when not yet expired" do
      ...
      expect(RedisClient.zcard("xque:pending:items")).to eq(1)
    end
    ...
    it "backs off failed jobs" do
      ...
      expect(RedisClient.zscore("xque:pending:items", job_id)).to be_between(RedisClient.time[0] + 55, RedisClient.time[0] + 65)
      expect(RedisClient.hlen("xque:jobs")).to eq(1)
    end
